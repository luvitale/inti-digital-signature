...
ipcMain.on("generate-private-key", async event => {
  ...
  try {
    ...
    const savedPrivateKeyPath = await dialogFileTransfer.save(privateKey, defaultPath)
...
ipcMain.on("generate-public-key", async (event, privateKeyPath) => {
  ...
  try {
    ...
    const savedPublicKeyPath = await dialogFileTransfer.save(publicKey, defaultPath)
...
ipcMain.on("sign", async (event, {privateKeyPath, fileToSignPath}) => {
  ...
  try {
    const initialSignedFilePath = await digitalSignature.sign(
      privateKeyPath, fileToSignPath, initialFilePath
    )
    const savedSignedFilePath = await dialogFileTransfer.move(
      initialSignedFilePath, defaultPath
    )
...
ipcMain.on("verify", async (
  event, {publicKeyPath, fileToVerifyPath, originalFilePath}
) => {
  try {
